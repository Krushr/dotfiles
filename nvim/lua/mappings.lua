...
map(
	"n",
	"<Leader>h",
	":let view=winsaveview()<CR>*:call winrestview(view)<CR>",
	{ noremap = false, silent = true, desc = "Highlight current word" }
)
map(
	"v",
	"<Leader>h",
	":<C-U>let view=winsaveview()<CR>gv*:<C-U>call winrestview(view)<CR>",
	{ noremap = false, silent = true, desc = "Highlight current word" }
)
